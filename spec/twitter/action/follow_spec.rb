...
describe TwitterAPI::Action::Follow do

  describe "#sources" do
    it "returns a collection of users who followed a user" do
      sources = TwitterAPI::Action::Follow.new(:sources => [{:id => 7505382}]).sources
      expect(sources).to be_an Array
      expect(sources.first).to be_a TwitterAPI::User
    end
    it "is empty when not set" do
      sources = TwitterAPI::Action::Follow.new.sources
  ...
  describe "#targets" do
    it "returns a collection containing the followed user" do
      targets = TwitterAPI::Action::Follow.new(:targets => [{:id => 7505382}]).targets
      expect(targets).to be_an Array
      expect(targets.first).to be_a TwitterAPI::User
    end
    it "is empty when not set" do
      targets = TwitterAPI::Action::Follow.new.targets
