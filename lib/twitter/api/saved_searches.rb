...
module TwitterAPI
  module API
    module SavedSearches
      ...
      # @return [Array<TwitterAPI::SavedSearch>] The saved searches.
      ...
      #     TwitterAPI.saved_search(16129012)
      ...
      def saved_searches(*args)
        ...
        if arguments.empty?
          objects_from_response(TwitterAPI::SavedSearch, :get, "/1.1/saved_searches/list.json", arguments.options)
        else
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [TwitterAPI::SavedSearch] The saved searches.
      # @param id [Integer] A Tweet IDs.
      # @param options [Hash] A customizable set of options.
      # @example Retrieve the data for a saved search owned by the authenticating user with the ID 16129012
      #   TwitterAPI.saved_search(16129012)
      def saved_search(id, options={})
        object_from_response(TwitterAPI::SavedSearch, :get, "/1.1/saved_searches/show/#{id}.json", options)
      end
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [TwitterAPI::SavedSearch] The created saved search.
      # @param query [String] The query of the search the user would like to save.
      # @param options [Hash] A customizable set of options.
      # @example Create a saved search for the authenticated user with the query "twitter"
      #   TwitterAPI.saved_search_create("twitter")
      def saved_search_create(query, options={})
        object_from_response(TwitterAPI::SavedSearch, :post, "/1.1/saved_searches/create.json", options.merge(:query => query))
      end
      ...
      def saved_search_destroy(*args)
        arguments = TwitterAPI::API::Arguments.new(args)
        arguments.flatten.threaded_map do |id|
          object_from_response(TwitterAPI::SavedSearch, :post, "/1.1/saved_searches/destroy/#{id}.json", arguments.options)
        end
      end
