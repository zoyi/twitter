...
module TwitterAPI
  module API
    module DirectMessages
      ...
      # @return [Array<TwitterAPI::DirectMessage>] Direct messages sent to the authenticating user.
      ...
      #   TwitterAPI.direct_messages_received
      def direct_messages_received(options={})
        objects_from_response(TwitterAPI::DirectMessage, :get, "/1.1/direct_messages.json", options)
      end
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<TwitterAPI::DirectMessage>] Direct messages sent by the authenticating user.
      # @param options [Hash] A customizable set of options.
      # @option options [Integer] :since_id Returns results with an ID greater than (that is, more recent than) the specified ID.
      # @option options [Integer] :max_id Returns results with an ID less than (that is, older than) or equal to the specified ID.
      # @option options [Integer] :count Specifies the number of records to retrieve. Must be less than or equal to 200.
      ...
      #   TwitterAPI.direct_messages_sent
      def direct_messages_sent(options={})
        objects_from_response(TwitterAPI::DirectMessage, :get, "/1.1/direct_messages/sent.json", options)
      end
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [TwitterAPI::DirectMessage] The requested messages.
      # @param id [Integer] A Tweet IDs.
      # @param options [Hash] A customizable set of options.
      # @example Return the direct message with the id 1825786345
      #   TwitterAPI.direct_message(1825786345)
      def direct_message(id, options={})
        options[:id] = id
        object_from_response(TwitterAPI::DirectMessage, :get, "/1.1/direct_messages/show.json", options)
      end
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<TwitterAPI::DirectMessage>] The requested messages.
      ...
      #   @example Return the direct message with the id 1825786345
      #     TwitterAPI.direct_messages(1825786345)
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [Array<TwitterAPI::DirectMessage>] Deleted direct message.
      # @overload direct_message_destroy(*ids)
      #   @param ids [Array<Integer>, Set<Integer>] An array of Tweet IDs.
      #   @example Destroys the direct message with the ID 1825785544
      #     TwitterAPI.direct_message_destroy(1825785544)
      # @overload direct_message_destroy(*ids, options)
      #   @param ids [Array<Integer>, Set<Integer>] An array of Tweet IDs.
      #   @param options [Hash] A customizable set of options.
      def direct_message_destroy(*args)
        threaded_object_from_response(TwitterAPI::DirectMessage, :post, "/1.1/direct_messages/destroy.json", args)
      end
      ...
      # @authentication Requires user context
      # @raise [TwitterAPI::Error::Unauthorized] Error raised when supplied user credentials are not valid.
      # @return [TwitterAPI::DirectMessage] The sent message.
      ...
      #   TwitterAPI.direct_message_create('sferik', "I'm sending you this message via @gem!")
      #   TwitterAPI.direct_message_create(7505382, "I'm sending you this message via @gem!")  # Same as above
      def direct_message_create(user, text, options={})
        merge_user!(options, user)
        options[:text] = text
        object_from_response(TwitterAPI::DirectMessage, :post, "/1.1/direct_messages/new.json", options)
      end
